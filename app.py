...
def build_report_stats(user=None, date_filter='', date_params=None):
    ...
    if branch_id:
        ...
        if date_filter:
            date_clause = f" WHERE {date_filter.replace('AND ', '')}"
            ...
            interview_date_clause = date_clause.replace('a.submitted_at', 'i.scheduled_date')
        else:
    ...
    if export_format == 'csv':
        ...
        if section == 'applicant_summary':
            ...
            if export_type == 'detailed' and applicant_summary_details:
                writer.writerow([])
                writer.writerow(['Detailed Applicant List'])
                writer.writerow(['Applicant Name', 'Email', 'Applications', 'Status', 'Last Activity'])
                for app in applicant_summary_details:
                    writer.writerow([
                        app.get('applicant_name', ''),
                        app.get('email', ''),
                        app.get('total_applications', 0),
            ...
            if export_type == 'detailed' and job_vacancy_details:
                writer.writerow([])
                writer.writerow(['Detailed Job List'])
                writer.writerow(['Job Title', 'Branch', 'Status', 'Applications', 'Posted Date'])
    ...
    try:
        ...
        try:
            ...
            if date_filter:
                ...
                date_clause_applicant = f" AND {date_filter.replace('AND ', '')}"
            ...
            # Get detailed applicant list
            if branch_id:
                detail_date_clause = f" AND {date_filter.replace('AND ', '')}" if date_filter else ""
                detail_query = f'''
                    SELECT 
                        ap.full_name AS applicant_name,
                        ap.email,
                        COUNT(DISTINCT a.application_id) AS total_applications,
                        MAX(a.status) AS current_status,
                        MAX(a.submitted_at) AS last_activity
                    FROM applicants ap
                    JOIN applications a ON ap.applicant_id = a.applicant_id
                    JOIN jobs j ON a.job_id = j.job_id
                    WHERE j.branch_id = %s{detail_date_clause}
                    GROUP BY ap.applicant_id, ap.full_name, ap.email
                    ORDER BY last_activity DESC
                    LIMIT 50
                '''
                detail_params = [branch_id] + (date_params_all if date_params_all else [])
            else:
                detail_where_clause = f"WHERE {date_filter.replace('AND ', '')}" if date_filter else ""
                detail_query = f'''
                    SELECT 
                        ap.full_name AS applicant_name,
                        ap.email,
                    ...
                    LIMIT 50
                '''
                detail_params = date_params_all if date_params_all else []
            ...
            # Format dates
            for app in applicant_summary_details:
                if app.get('last_activity'):
                    try:
                        if isinstance(app['last_activity'], (datetime, date_class)):
                            app['last_activity'] = app['last_activity'].strftime('%b %d, %Y %I:%M %p')
                        elif isinstance(app['last_activity'], str):
                            dt = datetime.strptime(app['last_activity'], '%Y-%m-%d %H:%M:%S')
                            app['last_activity'] = dt.strftime('%b %d, %Y %I:%M %p')
                    except:
                        pass
        except Exception as e:
        ...
        try:
            ...
            if date_filter:
                ...
                if job_where:
                    ...
                    job_where = f"WHERE {job_date_filter.replace('AND ', '')}"
        ...
        try:
            ...
            if date_filter:
                if hiring_where:
                    ...
                    hiring_where = f"WHERE {date_filter.replace('AND ', '')}"
        ...
        try:
            ...
            if has_activity_logs:
                cursor.execute("SHOW COLUMNS FROM activity_logs LIKE 'action_type'")
                has_action_type = cursor.fetchone() is not None
            else:
            ...
            if date_filter:
                if interview_where:
                    interview_where += f" {date_filter.replace('a.submitted_at', 'i.scheduled_date')}"
                else:
                    interview_where = f"WHERE {date_filter.replace('a.submitted_at', 'i.scheduled_date').replace('AND ', '')}"
            ...
            # Get status updates and reviews from applications (updated_at changes)
            ...
            if date_filter:
                if status_where:
                    ...
                    status_where = f"WHERE {date_filter.replace('AND ', '')}"
